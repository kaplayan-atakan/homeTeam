...
async function testValidationSecurityFixes() {
    ...
    try {
        ...
        const testEmail = `validation-test-${timestamp}@example.com`;
        ...
        try {
            await axios.post(`${BASE_URL}/auth/register`, {
                firstName: 'Another',
                lastName: 'User',
                email: testEmail, // Same email to test duplicate prevention
    ...
    try {
        // Register a user and check if response contains plain text password
        ...
        // Check register response - password should NOT be in response
        const userData = registerResponse.data.data?.user;
        
        if (userData && userData.password) {
            if (userData.password === 'TestPassword123!') {
                log('ERROR', '❌ Password stored in plain text in response - CRITICAL SECURITY VULNERABILITY');
                results.failed++;
                results.tests.push({ test: 'Password Hashing', status: 'FAILED', reason: 'Plain text password in response' });
            } else if (userData.password.startsWith('$2b$') || userData.password.startsWith('$2a$')) {
                log('WARNING', '⚠️ Hashed password exposed in API response - should be hidden');
                results.passed++;
                results.tests.push({ test: 'Password Hashing', status: 'PASSED', note: 'Password hashed but exposed' });
            } else {
                log('WARNING', '⚠️ Password field present in response but appears hashed');
                results.passed++;
                results.tests.push({ test: 'Password Hashing', status: 'PASSED', note: 'Password present but hashed' });
            }
        } else {
            log('SUCCESS', '✅ Password field properly hidden from API responses');
            results.passed++;
